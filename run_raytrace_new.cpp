...
/**
 * Atmospheric ray-tracing output.
 */
struct RayTraceOut
{
  RayTraceOut(double tht, double dist, double ran)
  : theta(tht), distance(dist), range(ran)
  {}

  /// The final elevation angle at the receiver [radians].
  double theta;
  /// The great-circle distance [km].
  double distance;
  /// The slant-range [km];
  double range;
};
 ...
 */
double
slant_range_cosine_2(double phi, double rho1, double rho2)
{
  //  Calculate the slant range using the law of cosines
  auto delrho = rho1 - rho2;
  auto fact = 2 * std::sin(phi / 2);
  auto slant_range = std::sqrt(delrho * delrho + (rho1 * fact) * (rho2 * fact));
 ...
 */
RayTraceOut
raytrace(const RayTrace& rt, double height_tmit, double height_recv, double theta_init)
{
  ...
  if (theta_neg)
  {
    ...
    //auto delta_phi = (theta[i] - theta[im1]) / (rt.pt[i].a + 1.0);
    //slant_range += slant_range_cosine_2(delta_phi, rt.pt[i].radius, rt.pt[im1].radius);
    theta[i_save] = ct;
    auto nk = i_save + 1;
  ...
  const auto distance = rt.radius_surf * ca; // We need to return this too.
  ...
  x = std::atan(x);
//std::cout << "slant_range? " << x << " nope\n";
  auto cx = theta_curr - x; // Unused?

  return RayTraceOut(theta_final, distance, x/*slant_range*/);
}
 ...
 * @param[inout] theta_final Output takeoff angle such that the ray arrives at the given receiver height and distance.
 * @return The output ray parameters at the receiver.
 */
RayTraceOut
raytrace(const RayTrace& rt, double height_tmit, double height_recv, double distance, double theta_init, double tol,
         double& theta_final)
{
  auto theta_0 = theta_init;
  auto theta_1 = theta_0 + 0.005;
  auto out0 = raytrace(rt, height_tmit, height_recv, theta_0);
  auto out1 = raytrace(rt, height_tmit, height_recv, theta_1);
  for (int i = 0; i < 10; ++i)
  {
    const auto jacobian_inv = (theta_1 - theta_0) / (out1.distance - out0.distance);
    const auto theta_2 = theta_1 - jacobian_inv * (out1.distance - distance);
    auto out2 = raytrace(rt, height_tmit, height_recv, theta_2);
    theta_0 = theta_1;
    out0 = out1;
    theta_1 = theta_2;
    out1 = out2;
    if constexpr (!QUIET)
      std::cout << ' ' << i+1 << ' ' << theta_2 << ' ' << out2.distance << '\n';
    if (std::abs(out1.distance - distance) < tol)
      break;
  }

  // In this case, the transmit elevation angle - the actual solve-for - is returned.
  theta_final = theta_1;
  return out1;
}
...
main()
{
  ...
  raytrace_write(rt);
  auto ry = raytrace(rt, hp1, hp2, -1.56);
  auto ds0 = ry.distance;
  if constexpr (!QUIET)
  {
    std::cout << "ry = " << ry.theta << '\n';
    std::cout << "qqd = " << ry.distance << '\n';
  }
  ...
  raytrace_write(rt);
  auto ry2 = raytrace(rt, zero, hp2, zero);
  auto horizon_recv = ry2.distance;
  if constexpr (!QUIET)
  {
    std::cout << "ry2 = " << ry2.theta << '\n';
    std::cout << "qqd = " << ry2.distance << '\n';
  }

  if constexpr (!QUIET)
    std::cout << '\n';
  auto ry1 = raytrace(rt, zero, hp1, zero);
  auto horizon_tmit = ry1.distance;
  if constexpr (!QUIET)
  {
    std::cout << "ry1 = " << ry1.theta << '\n';
    std::cout << "qqd = " << ry1.distance << '\n';
  }

  // Made up....
  if constexpr (!QUIET)
    std::cout << '\n';
  auto ray = raytrace(rt, hp1, hp2, 0.05);
  auto dist = ray.distance;
  if constexpr (!QUIET)
  {
    std::cout << "ray = " << ray.theta << '\n';
    std::cout << "qqd = " << ray.distance << '\n';
  }

  // Made up point downward....
  if constexpr (!QUIET)
    std::cout << '\n';
  auto ray2 = raytrace(rt, hp1, hp2, -0.05);
  auto dist2 = ray2.distance;
  if constexpr (!QUIET)
  {
    std::cout << "ray2 = " << ray2.theta << '\n';
    std::cout << "qqd2 = " << ray2.distance << '\n';
  }
  ...
  double theta_f;
  auto ray_f = raytrace(rt, hp1, hp2, distance_f, theta_0, 0.001, theta_f);
  if constexpr (!QUIET)
  {
    std::cout << "distance_f = " << distance_f << '\n';
    std::cout << "theta_0 = " << theta_0 << '\n';
    std::cout << "theta_f = " << theta_f << '\n';
    std::cout << "distance = " << ray_f.distance << '\n';
    std::cout << "range = " << ray_f.range << '\n';
  }
  ...
  ray_f = raytrace(rt, hp1, hp2, distance_f, theta_0, 0.001, theta_f);
  if constexpr (!QUIET)
  {
    std::cout << "distance_f = " << distance_f << '\n';
    std::cout << "theta_0 = " << theta_0 << '\n';
