...
C     -------------PROGRAM START WITH CARD 1--------------------- 
  100 READ(IN,9)IK,IO,IJ,ILB,KK,IA,TT,DMAX,JC,IGPH  
      JB=1              
      IF (IGPH .GT. 1) JA=1           
      RQD=DMAX          
      WRITE(IOT,4)      
      ...
      NOC=0             
C     -----------------INPUT OF CARD 2--------------------------- 
      READ(IN,7)HLA,IFA,JT,IPL,T1T,HLPBW,SUR,IZ,STS,KD,KE,DHSI,DHOI,HHOI        
     X,IDG,IMN,ISEC,DCI,HCI,ICC       
      CALL PAGE(-1)     
      WRITE(IOT,5)      
...
C     ---------------INPUT OF CARD 3 IF NECESSARY---------------- 
      READ(IN,8)HAI,IAA,JS,NPL,T2T,H2PBW,ENO,F,EIRP,HPFI,KSC,TP,SCK,ISS,        
...
*     NK=43-((20+IA)/2)               
      ...
      UN=UNH(IK)        
      UD=UND(IK)        
      WRITE(IOT,'(A)')BLANK(1:NK)//LL(IO)//TT       
      IF(KSC.LT.1) KSC=3              
      IF(IJ.GT.0) GO TO 80            
      H2=HAI*CFK(IK)    
      WRITE(IOT,701)HAI,UN            
   81 IF(HAI.GT.150000.) WRITE(IOT,712)             
      IF(HAI.LT.500.) WRITE(IOT,713)                
      IF(HAI.LT.1.5) WRITE(IOT,714)   
...
*      CALL RYTRA(DUM)                 
   ...
   59 DHEL=EAL*CKM(IK)                
      ...
      HFI=HFS*CKM(IK)                 
      HFRI=HTE*CKM(IK)                
      WRITE(IOT,702)HFP,UN            
      IF(HFI.LT.0.) GO TO 825         
      IF(HFI.LT.1.5) WRITE(IOT,716)   
      WRITE(IOT,703)FREK              
      WRITE(IOT,5)      
      WRITE(IOT,704)    
      WRITE(IOT,720)    
      ...
      IF(JS.GT.0) WRITE(IOT,840)      
      WRITE(IOT,715)HPFI,UN           
      IF(IO-2) 791,792,793            
  791 PIRP=EIRP         
      WRITE(IOT,851) EIRP             
      GO TO 817         
  ...
  817 WRITE(IOT,721)    
      ...
      IF(T1T.GE..01.OR.T1T.LE.(-.01)) WRITE(IOT,850)T1T           
      IF(JT.GT.0) WRITE(IOT,840)      
...
C     -------------COUNTERPOISE PARAMETERS CONVERTED------------- 
      ...
      DCW=DCI*CFK(IK)                 
      HCW=HCI*CFK(IK)                 
      WRITE(IOT,706)DCI,UN,HCI,UN,TSC(ICC)          
...
*     ENCODE(3,140,ADG)IDG            
  ...
  783 CONTINUE          
   ...
   72 IF(DLT.LT.(.1*DLST).OR.DLT.GT.(3.*DLST)) WRITE(IOT,809)     
      IF(TET.GT..20943951) WRITE(IOT,810)           
   ...
   10 TO2=-TO1          
      ...
      DLR=QQD           
   18 DML=DLR+DLT       
   70 WRITE(IOT,708)DHOI,UD,PDS,ADG,IMN,ISEC,PTS,HHOI,UN,PHS      
C     ----------------------------------------------------------- 
      IF(JO.LE.0) GO TO 79            
      IF(IOS.LT.0) GO TO 84           
      WRITE(IOT,796)IOS               
   85 IF(IPK.GT.0) WRITE(IOT,795)     
   79 CONTINUE          
      IF(IZ.LE.0) GO TO 76            
      WRITE(IOT,794)IZ,STS            
   76 CONTINUE          
      WRITE(IOT,709)EART,UD,ENO       
      IF(ILB.GT.0) GO TO 762          
      WRITE(IOT,778)    
  763 CONTINUE          
      CALL SIG(KSC,F)                 
      WRITE(IOT,744)TSC(KSC)          
      ...
      RMS=0.            
  177 CONTINUE          
      WRITE(IOT,710)SUR,UN,DHSI,UN    
      WRITE(IOT,729)VYD(KK)           
      DE=0.             
      CALL VARIB(DE)    
      IF(IO.EQ.2) WRITE(IOT,776)PDCON               
      WRITE(IOT,724)PAS(2)            
      IF(DMAX.GT.1000.) DMAX=1000.    
      IF(ICAR.GT.0) WRITE(IOT,800)    
...
*     WRITE(IOT,757)QMD               
      ...
      WRITE(IOT,6)      
      IF(IJ.GT.0) GO TO 82            
      WRITE(IOT,731)HAI,UN,H2         
      WRITE(IOT,736)DHEI,UN,EAC,PDH   
      WRITE(IOT,728)H2,EAC,HRP,HRE    
   83 CONTINUE          
      WRITE(IOT,732)HLA,UN,H1         
      WRITE(IOT,736)DHEL,UN,EAL,PDH   
      WRITE(IOT,820)H1,EAL,HRP,HTE    
      WRITE(IOT,733)F,FREK            
      WRITE(IOT,734)AOI,GAO,PXH       
      WRITE(IOT,735)AWI,GAW,PXH       
      WRITE(IOT,737)EIRP,PIRP         
      WRITE(IOT,738)IFA               
      IF(HLPBW.GT..01) WRITE(IOT,756)HLPTW          
      IF(T1T.GE..01.OR.T1T.LE.(-.01)) WRITE(IOT,850)T1T           
      IF(JT.GT.0) WRITE(IOT,840)      
      ...
      WRITE(IOT,739)DCI,UN,DCW        
      WRITE(IOT,740)HCI,UN,HCW        
      WRITE(IOT,741)ICC,TSC(ICC)      
  754 CONTINUE          
      WRITE(IOT,742)HFRI,UN,HTE       
      WRITE(IOT,743)IPL,POL(IPL)      
      WRITE(IOT,745)PDS,DHOI,UD,DLT   
      WRITE(IOT,746)PTS,ADG,IMN,ISEC,TET            
      WRITE(IOT,747)PHS,HHOI,UN,HLT   
      WRITE(IOT,748)ENO,ENS           
      WRITE(IOT,726)EART,UD,EFRTH     
      WRITE(IOT,749)SUR,UN,ETS        
      WRITE(IOT,750)DHSI,UN,DH        
      WRITE(IOT,751)KSC,TSC(KSC)      
      IF(KSC.EQ.1.OR.KSC.EQ.5) WRITE(IOT,172)ISS,SSS(ISK),RMS,UN,TP             
      IF(ILB.GT.0) GO TO 764          
      WRITE(IOT,785)    
  765 IF(IO.EQ.2) WRITE(IOT,775)PDCON               
      IF(IO.EQ.3) WRITE(IOT,818)PIRP                
      IF(IO.EQ.1) WRITE(IOT,819)      
      WRITE(IOT,729)VYD(KK)           
      CALL VARLB(DE)    
      WRITE(IOT,724)PAS(2)            
      WRITE(IOT,5)      
...
C     -----------------------WRITE STATEMENTS-------------------- 
      WRITE(IOT,772)    
      WRITE(IOT,767)HTE,HRE,D,DLT,DLR,ENS,EFRTH,FREK,ALAM,TET,TER 
      WRITE(IOT,773)    
      WRITE(IOT,768) HT,HR ,DH,AED,SLP,DLST,DLSR,DNM,ALFS,AMD,DZR,WRH           
      WRITE(IOT,761)PRH,AWD,SWP       
      WRITE(IOT,5)      
      CALL PAGE(6)      
C     ----------------------------------------------------------- 
...
C     ------------------TROPOSPHERIC MULTIPATH------------------- 
   ...
   75 HTE=HP1           
      ...
      GO TO 73          
   80 H2=HAI*CMK(IK)    
      WRITE(IOT,723) HAI,UND(IK)      
      GO TO 81          
   82 WRITE(IOT,727)HAI,UND(IK),H2    
      WRITE(IOT,725)DHEI,UND(IK),EAC,PDH            
      WRITE(IOT,771)H2,EAC,HRP,HRE    
      GO TO 83          
   84 WRITE(IOT,797)    
      GO TO 85          
   92 DO 95 I=1,35      
...
*     WRITE (1,172)ISS,SSS(ISK),RMS,UN,TP           
      WRITE (IOT,172)ISS,SSS(ISK),RMS,UN,TP         
      IF(KSC.EQ.1) WRITE(IOT,175)     
      GO TO 177         
  174 RMS=SCK           
      WRITE(IOT,176) RMS,UN           
      SCK=SCK*CFM(IK)                 
      GO TO 177         
  199 PL=PL-RS(18)      
      DO 198 I=1,35     
